...
The Sage Studio "Getting Started" documentation is **critically misaligned** with the current architecture. All documentation references the **OLD** system (`@ecosystem/design-system` + atomic design) while the codebase is **actively migrating** to the **NEW** system (`@thesage/ui` + functional organization).
...
- ✅ **Phase 4 COMPLETE** (2026-01-15): Legacy Migration
  - **Infrastructure:** Full infrastructure created in @thesage/ui (syntax-parser, stores, hooks, providers)
  - **Subpath Exports:** Configured for improved DX (`@thesage/ui/tokens`, `@thesage/ui/hooks`, `@thesage/ui/utils`, `@thesage/ui/providers`)
...
ecosystem/
├── packages/
│   ├── ui/                      # @thesage/ui - Component library (NEW)
│   │   └── src/
│   │       ├── components/
...
// SHOULD BE:
pnpm add @thesage/ui @thesage/tokens
```
...
// SHOULD BE:
import { useTheme, useMotion } from '@thesage/ui/hooks';
```
...
**File:** `apps/web/app/components/studio/AddingComponentsSection.tsx`
...
- Registry location remains: `apps/web/app/components/lib/component-registry.tsx`
- Import path changes to: `import { ComponentName } from '@thesage/ui'`

**Molecule Section (Lines ~245-273):**
```typescript
// CURRENT (WRONG):
"Follow the same steps as adding an Atom..."
import { Input, Button } from '@thesage/ui'; // This is correct
...
**File:** `apps/web/app/components/studio/ArchitectureSection.tsx`
...
**File:** `apps/web/app/components/studio/CommonPatternsSection.tsx`
...
// SHOULD BE:
import { useForm } from '@thesage/ui/hooks';
import { TextField, Button } from '@thesage/ui';
// Note: Need to verify if TextField exists in @thesage/ui or needs to be created
```
...
// SHOULD BE:
import { useToast } from '@thesage/ui/hooks';
import { Button, ToastProvider } from '@thesage/ui';
```
...
// SHOULD BE:
import { Dialog, Button } from '@thesage/ui';
...
Before updating code examples, verify these components exist in `@thesage/ui`:
...
- [ ] Run Studio app: `cd apps/web && pnpm dev`
...
- [ ] Build Studio app: `cd apps/web && pnpm build`
...
**Strategy Document:** `apps/web/docs/SAGE_DESIGN_SYSTEM_STRATEGY.md`
...
| **Update all examples to @thesage/ui** | Future-facing, prepares developers for production usage |
...
1. **Component naming:** Is "Modal" renamed to "Dialog" in @thesage/ui? Check shadcn naming conventions.
2. **Hook locations:** Are all hooks exported from `@thesage/ui/hooks` or root `@thesage/ui`?
3. **Provider locations:** Is `ThemeProvider` at `@thesage/ui/providers` or `@thesage/ui`?
4. **TextField existence:** Does `TextField` molecule exist in @thesage/ui or need to be created?
...
✅ All code examples use `@thesage/ui` and `@thesage/tokens`
...
✅ Build commands reference `@thesage/ui` not `@ecosystem/design-system`
