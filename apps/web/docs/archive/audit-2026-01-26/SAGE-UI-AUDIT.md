...
- `apps/web/app/components/studio/OverviewSection.tsx` - Getting Started content
- `apps/web/app/components/studio/McpSection/` - MCP Server tabs
...
1. **Installation verification** - Attempted `npm install @thesage/ui` and `npm install @thesage/mcp`
...
**@thesage/ui and @thesage/mcp packages are NOT published to npm.** The first step of the Getting Started guide instructs users to `pnpm add @thesage/ui`, which returns a 404 error. This makes 100% of the documentation incorrect for external users.
...
```bash
$ npm view @thesage/ui
# Error: 404 Not Found - '@thesage/ui@*' could not be found

$ npm view @thesage/mcp
# Error: 404 Not Found - '@thesage/mcp@*' could not be found
```
...
3. Publish `@thesage/ui` v0.0.5 to npm
4. Publish `@thesage/mcp` v0.1.0 to npm
...
**Files to Update:**
- `apps/web/app/components/studio/OverviewSection.tsx` (lines 875-996)
- `apps/web/app/components/studio/McpSection/InstallationTab.tsx` (lines 17-126)
- Add new file: `apps/web/docs/MONOREPO-SETUP.md`
...
**Files to Update:**
- `apps/web/app/components/studio/OverviewSection.tsx` (multiple locations)
- `apps/web/app/components/studio/McpSection/OverviewTab.tsx` (line 48)
...
**Files to Update:**
- `apps/web/app/components/studio/OverviewSection.tsx` (add before line 875)
...
**Problems:**
1. `npx @thesage/mcp` will try to download from npm (404 - not published)
...
Add note:
```markdown
> ⚠️ **Important:** `@thesage/mcp` is not yet published to npm. To use the MCP server:
...
**Files to Update:**
- `apps/web/app/components/studio/McpSection/InstallationTab.tsx` (lines 55-72)
...
**Files to Update:**
- `apps/web/app/components/studio/OverviewSection.tsx` (reorder sections or add context)
...
export default {
  content: [
    ...
    './components/**/*.{js,ts,jsx,tsx,mdx}',
    './node_modules/@thesage/ui/**/*.{js,ts,jsx,tsx}',  // Required
  ],
  theme: {
    extend: {},
  },
...
- "Module not found: @thesage/ui" → Peer deps not installed
...
**Solutions:**
- ✅ Add `./node_modules/@thesage/ui/**/*.{js,ts,jsx,tsx}` to Tailwind content
...
**Symptoms:**
```
Cannot find module '@thesage/ui' or its corresponding type declarations
```
...
**Solutions:**
- ✅ Run `pnpm install @thesage/ui`
- ✅ If using monorepo: `pnpm build --filter @thesage/ui`
- ✅ Check import: `import { Button } from '@thesage/ui'` not `'@thesage/ui/Button'`
...
**Symptoms:**
```
npm WARN @thesage/ui requires a peer of react@* but none is installed
```
...
- 📖 Check the [Usage Guide](https://github.com/shalomormsby/ecosystem/blob/main/apps/web/docs/SAGE_DESIGN_SYSTEM_STRATEGY.md)
...
**Files to Update:**
- `apps/web/app/components/studio/OverviewSection.tsx` (add new section after Quick Start)
...
**Files to Update:**
- `apps/web/app/components/studio/OverviewSection.tsx` (update all internal links)
...
```bash
mv apps/web/docs/SAGE_DESIGN_SYSTEM_STRATEGY.md \
   apps/web/docs/USAGE_GUIDE.md
```
...
MCP server registry has 36 components, but 60+ are exported from `@thesage/ui`.
...
**Files to Update:**
- `apps/web/app/components/studio/OverviewSection.tsx` (add after Quick Start)
...
**Files to Update:**
- `apps/web/app/components/studio/OverviewSection.tsx` (add at top of Quick Start)
...
```bash
pnpm dev --filter @thesage/mcp  # Rebuilds on file changes
```
```

**Files to Update:**
- `apps/web/app/components/studio/McpSection/InstallationTab.tsx` (add new section)
...
**Documentation Files:**
- Getting Started content: `apps/web/app/components/studio/OverviewSection.tsx`
- MCP Server content: `apps/web/app/components/studio/McpSection/`
- Navigation: `apps/web/app/docs/page.tsx`
...
```bash
npm view @thesage/ui  # Should show package info OR 404
```
...
cd test-sage-ui
npm install @thesage/ui react framer-motion tailwindcss
